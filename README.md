...
- **近30天趋势研究**：先用内置浏览器打开小红书站内搜索页、公开笔记页或用户提供的站内链接；第一轮必须展示 `browser_route`、实际 URL、页面标题、页面身份、搜索词、搜索状态和前 3 条可见结果摘要。前 3 条仅是访问证据预览，不是 Top20。证据确认后必须继续滚动/翻页/扩展检索词，输出完整 `top20_samples`（不足20条时如实说明）；代表文章只能从完整候选集里另选 1 篇做补充深读，不能替代 Top20。站内访问失败须经用户明确授权才能降级为公开网页调研。
- **页面可读性恢复**：打开界面不等于读取成功；读取失败时必须等待、刷新、内置浏览器新标签页重开并读取诊断，每次记录尝试、错误和恢复动作。截图可见但文本/DOM 失败时使用截图证据；登录/验证码请求人工处理，四次恢复前不得切换外置浏览器或桥接。
- **内容生产与预审**：从趋势样本提炼当前时点概要，再生成标题、正文、话题和配图建议，并输出 `可发 / 需修改 / 暂不建议发`
- **发布模式与五要素收集、补齐与校验**：先选择图文模式或长文模式；标题（≤20 字）、备选标题、正文（保留换行与 emoji；图文模式最终提交字符串 ≤1000 个字符，长文模式不执行本技能的 1000 字符校验）、话题标签（通常 5-7 个）、封面图（至少 1 张）
...
```powershell
# 素材文件方式
python scripts/build_publish_pack.py --mode image_text --source "<工作区>\小红书发布素材\文字\01_工位免费3个月\01_工位免费3个月.txt" --time "明天 10:10"

# 直传参数方式
python scripts/build_publish_pack.py --mode image_text --title "标题" --body "正文" --tags "#话题1 #话题2" --cover "封面.jpg" --theme "主题" --num "02" --time "立即发布"

# 长文模式：不启用本技能的 1000 字符正文上限
python scripts/build_publish_pack.py --mode long_form --title "长文标题" --body "较长正文" --tags "#话题1 #话题2" --cover "封面.jpg" --theme "主题" --num "03" --time "立即发布"
```
...
| 2. 趋势研究 | 仅给主题时先打开小红书站内页面；前3条只做访问证据预览，随后继续采集并去重 | 输出完整 `top20_samples`（按可见点赞排序，或说明严格不足原因）；代表文章只能作为 Top20 后的补充深读 |
...
| 备选标题 | 可选；缺失不阻塞 |
| 发布模式 | 必选图文模式或长文模式；未选择不得生成正文或发布包 |
...
|---|---|
| `--source` | 素材 txt 路径 |
| `--mode` | 发布模式：`image_text` 图文模式（正文≤1000 字符）；`long_form` 长文模式（不启用本技能的 1000 字符正文上限），默认 `image_text` 仅为脚本兼容旧命令；技能对话中必须先让用户选择 |
...
- **正文按模式校验**：图文模式建议控制在 800-950 个字符，最终提交字符串（包含换行和 emoji，不含标题、话题标签、封面路径和发布提示）必须 ≤1000 个字符；长文模式不执行本技能的 1000 字符上限。生成后和发布前都要按所选模式重新统计，禁止静默截断；

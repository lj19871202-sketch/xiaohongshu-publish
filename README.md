...
- **近30天趋势研究**：先打开小红书站内搜索页、公开笔记页或用户提供的站内链接，再按可见点赞筛选 Top20；同时分析可实际读取的标题、正文、封面、轮播图和公开视频关键帧。站内访问失败须经用户明确授权才能降级为公开网页调研，样本不足时如实说明
...
- 趋势研究必须先用内置浏览器打开小红书站内搜索页、公开笔记页或用户提供的站内链接，并记录 `browser_route: in_app` 和站内访问结果；内置浏览器明确技术失败后，才可依次尝试外置 Edge/Chrome、Playwright 桥接。所有站内路径失败后，只有用户明确授权才可使用公开网页调研，并须记录失败原因和局限；不得先用互联网搜索。

...
- **五要素收集、补齐与校验**：标题（≤20 字）、备选标题、正文（保留换行与 emoji；最终提交字符串必须 ≤1000 个字符，超限先精简）、话题标签（通常 5-7 个）、封面图（至少 1 张）
...
| 备选标题 | 可选；缺失不阻塞 |
| 正文 | 必填；按最终提交字符串统计，包含换行与 emoji，必须 ≤1000 个字符；超限先精简，不得生成发布包 |
...
- **正文硬上限**：生成正文建议控制在 800-950 个字符，最终提交字符串（包含换行和 emoji，不含标题、话题标签、封面路径和发布提示）必须 ≤1000 个字符。生成后和发布前都要重新统计；超过上限必须先精简并复核，禁止静默截断、生成发布包或进入平台填写；

...
研究和发布都严格按以下顺序选择，不能跳级：

1. **内置浏览器（第一优先级）**：必须先用当前会话的内置浏览器尝试打开和操作页面，并记录结果。
2. **外置浏览器（第二优先级）**：只有内置浏览器发生明确技术失败（工具不可用、导航调用报错、页面无法加载等）后，才切换 Edge 或 Chrome，并记录切换原因。
3. **桥接方案（最后浏览器路径）**：只有内置浏览器和外置浏览器都明确不可用时，才使用下方方案三「Playwright + Edge 调试端口」。已有登录态、桥接已启动、外置浏览器更方便，都不是跳过前一级的理由。

登录、扫码、验证码、人工确认或页面内容需要等待时，不算技术失败；应停留在当前浏览器请求用户处理，不得自动跳级。
...
- 进入任何填写路径前，先按所选模式统计最终提交正文字符串（包含换行和 emoji，不包含标题、话题标签和发布提示）。图文模式必须 ≤1000，超限时先精简并复核，禁止静默截断、填写或提交；长文模式不执行本技能的 1000 字符校验，但仍需遵守页面实际可用限制。

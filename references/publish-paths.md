...
| 会话具备 Computer Use / 浏览器工具 | 浏览器自动化（仍建议发布前人工确认） |
...
- 助手侧：整理「直接复制版」内容（标题/正文/话题/封面路径），生成一段发给 ChatGPT 的指令：
  打开 creator.xiaohongshu.com → 检查登录（未登录提示扫码）→ 新建笔记填内容 → 设置定时发布后先确认 → 用户确认后手动点发布。
- ChatGPT 会话需已安装浏览器扩展并授权；发布按钮始终留给用户。

## 方案三：Playwright + Edge 调试端口（js_repl 关闭时也能自动化）

> 适用场景：当前会话没有暴露 Node REPL / Computer Use 工具（`[features] js_repl = false`），
